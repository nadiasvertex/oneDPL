...
template <typename Iter, sycl::access::mode mode = sycl::access::mode::read_write>
auto
get_host_access(Iter it) -> decltype(it.get_buffer().template get_access<mode>(
    it.get_buffer().get_count() - (it - oneapi::dpl::begin(it.get_buffer())), it - oneapi::dpl::begin(it.get_buffer())))
{
